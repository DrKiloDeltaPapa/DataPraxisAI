...
export default Admin
  ...
  return (
    <div className='mx-auto max-w-6xl px-4 py-8'>
      <h2 className='text-3xl font-bold mb-2'>Admin Dashboard</h2>
      <p className='text-gray-600 mb-6'>Generate blog content using RAG or manage embeddings.</p>

      {/* RAG Status Box */}
      {ragStatus && (
        <div className={`mb-6 p-4 rounded ${ragStatus.config_valid ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
          <div className='flex items-center gap-2 mb-2'>
            <span className={`text-sm font-semibold ${ragStatus.config_valid ? 'text-green-700' : 'text-yellow-700'}`}>
              {ragStatus.config_valid ? '✓ RAG Ready' : '⚠ Limited RAG'}
            </span>
          ...
          <p className='text-xs text-gray-600'>
            {ragStatus.config_valid ? 'Lakehouse available. Full RAG enabled.' : `Issue: ${ragStatus.config_error}`}
          </p>
        </div>
      )}
                  ...
                  <ul className='space-y-2'>
                    {generatedBlog.sources.map((source, i) => (
                      <li key={i} className='text-sm'>
                        <div className='font-medium'>
                          {source.title || 'Untitled'}
                          <span className='text-xs text-gray-500 ml-2'>(score: {(source.score * 100).toFixed(1)}%)</span>
                        </div>
                        <div className='text-xs text-gray-600'>
                          {source.author && `By ${source.author} • `}
                          {source.path && `Path: ${source.path}`}
